    ...
    >
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '6px' }}>
        <div className="entry-card-topline">
          ...
          {isWithinRange && <span className="entry-range-badge within-range">วีคปัจจุบัน</span>}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <span className={`type-pill ${type.tone}`}>{getEntryTypeLabel(entry)}</span>
          <span style={{ fontSize: '12px', color: 'var(--muted)', fontWeight: 'bold', flexShrink: 0 }}>
            {hours.toFixed(1)} ชม.
          </span>
          <p
            style={{
